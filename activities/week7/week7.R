...
# Group: Rithu Uppalapati, Yanyu Long, Yawen Hu 
# 79: -------------------------------------------------------------------------
...
recs_core = 
  ...
  transmute( 
    ...
    # grouping factor
    region = factor(REGIONC, levels = 1:4, labels = c('Northeast', 'Midwest', 'South', 'West')),
...
temps_by_region1 = recs_core %>% 
  group_by(region) %>%
  summarize(across(starts_with("temp"), ~weighted.mean(.x, na.rm = TRUE, weight = weight), .names = "avg_{.col}"))
...
recs_mean0 = function(df, vars) {
  ...
  df %>% 
    summarize(across(.cols = all_of(vars), ~weighted.mean(.x, na.rm = TRUE, weight = weight), .names = "avg_{.col}"))
  df
}
...
if ( TEST ) {
  ...
  recs_mean1(recs_core, vars = c('temp_home', 'temp_gone', 'temp_night'), group = "region")
